...
5. While the peer is running, do host analysis independently. In `ultra`, use host-local subagents for independent lenses such as mapper, reviewer, researcher, architect, security-auditor, debugger, test-strategist, and verifier when available.
...
8. After host first pass and peer output exist, run an advisory host-local adjudicator in `ultra`. The adjudicator receives the neutral brief, host first pass, peer report, helper reports, and claim matrix. It must not call the other product or invoke Agent Collab.
9. Verify important peer, helper, and adjudicator claims yourself before final synthesis.
10. Snapshot git state after the run and report unexpected mutations, especially when `edit_allowed=false`.
11. Synthesize the final result in Markdown.
