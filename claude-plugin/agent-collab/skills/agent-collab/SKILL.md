...
9. While the peer runs, do independent challenge-first host analysis. In `ultra`, spawn Claude-local subagents for independent lenses when useful: mapper, reviewer, researcher, architect, security-auditor, debugger, test-strategist, verifier. Give each subagent only the neutral brief and its lens. Tell every helper: "Do not invoke Agent Collab, `$agent-collab`, `/agent-collab`, host/peer CLIs, or cross-product peer commands." Do not call `status --wait` during independent host analysis; repeated status polling is not part of the normal flow.
...
11. Finish the run. `finish` is the normal synchronization point after `host-first-pass.json`; it waits responsively for peer artifacts, normalizes the report, and returns without repeated status polling:
...
14. Verify important claims yourself and synthesize the final answer using `"$runtime_root/references/synthesize.md"`. Do not treat peer agreement as proof.
