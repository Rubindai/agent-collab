...
model: opus
effort: max
allowed-tools: Bash Read Grep Glob WebSearch WebFetch Edit Write MultiEdit Task
---
...
8. While the peer runs, do independent host analysis. In `ultra`, spawn Claude-local subagents for independent lenses when useful: mapper, reviewer, researcher, architect, security-auditor, debugger, test-strategist, verifier. Give each subagent only the neutral brief and its lens. Do not call `status --wait` during independent host analysis; repeated status polling is not part of the normal flow.
...
11. In `ultra`, use a Claude-local advisory adjudicator after the host first pass, peer report, helper reports, and claim matrix exist. The adjudicator is advisory only and must not call Codex, cross-agent peer commands, or Agent Collab.
